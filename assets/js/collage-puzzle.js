...
(function () {
    ...
    function trySnap(anchor) {
        if (anchor.dataset.flipped !== '1') return; const ra = getStyleNum(anchor, '--rot'); const ar = anchor.getBoundingClientRect(); const ax = ar.left + ar.width / 2, ay = ar.top + ar.height / 2; const aw0 = anchor.offsetWidth, ah0 = anchor.offsetHeight; const ca = Math.cos(ra * Math.PI / 180), sa = Math.sin(ra * Math.PI / 180); let snapped = false; const others = [...document.querySelectorAll('.polaroid')].filter(o => o !== anchor && o.dataset.flipped === '1' && ensureGroup(o) !== ensureGroup(anchor)); for (const o of others) {
            const rb = getStyleNum(o, '--rot'); let dAng = ((rb - ra + 90) % 180 + 180) % 180 - 90; if (Math.abs(dAng) > SNAP.ang) continue; const diff360 = ((rb - ra) % 360 + 360) % 360; const near180 = Math.abs(diff360 - 180) <= SNAP.ang; const br = o.getBoundingClientRect(); const bx = br.left + br.width / 2, by = br.top + br.height / 2; const bw0 = o.offsetWidth, bh0 = o.offsetHeight; const dxw = bx - ax, dyw = by - ay; let dx = dxw * ca + dyw * sa; let dy = -dxw * sa + dyw * ca; const horizontal = Math.abs(dx) >= Math.abs(dy); const idealDX = horizontal ? Math.sign(dx) * ((aw0 + bw0) / 2 - SNAP.gap) : 0; const idealDY = horizontal ? 0 : Math.sign(dy) * ((ah0 + bh0) / 2 - SNAP.gap); const tolX = SNAP.frac * ((aw0 + bw0) / 2); const tolY = SNAP.frac * ((ah0 + bh0) / 2); const ex0 = idealDX - dx; const ey0 = idealDY - dy; if (Math.abs(ex0) > tolX || Math.abs(ey0) > tolY) continue; const setA = membersOf(anchor); const setB = membersOf(o); const both = new Set([...setA, ...setB]);
            ...
            const sA = getSides(anchor), sB = getSides(o);
            if ((sA[sideA] && sA[sideA].node !== o) || (sB[sideB] && sB[sideB].node !== anchor)) continue;
            withoutAnim(both, () => { const targetX = ax + (idealDX * ca - idealDY * sa); const targetY = ay + (idealDX * sa + idealDY * ca); const dAng2 = ((ra - rb + 90) % 180 + 180) % 180 - 90; const rad = dAng2 * Math.PI / 180; const cosA = Math.cos(rad), sinA = Math.sin(rad); let minL = Infinity, minT = Infinity, maxR = -Infinity, maxB = -Infinity; setB.forEach(n => { const r = n.getBoundingClientRect(); minL = Math.min(minL, r.left); minT = Math.min(minT, r.top); maxR = Math.max(maxR, r.right); maxB = Math.max(maxB, r.bottom); }); const pivotX = (minL + maxR) / 2, pivotY = (minT + maxB) / 2; const obr = o.getBoundingClientRect(); const ocx = obr.left + obr.width / 2, ocy = obr.top + obr.height / 2; const orx = pivotX + ((ocx - pivotX) * cosA - (ocy - pivotY) * sinA); const ory = pivotY + ((ocx - pivotX) * sinA + (ocy - pivotY) * cosA); const tX = Math.round(targetX - orx); const tY = Math.round(targetY - ory); setB.forEach(n => { const r = n.getBoundingClientRect(); const cx = r.left + r.width / 2, cy = r.top + r.height / 2; const rx = pivotX + ((cx - pivotX) * cosA - (cy - pivotY) * sinA); const ry = pivotY + ((cx - pivotX) * sinA + (cy - pivotY) * cosA); const deltaX = (rx + tX) - cx; const deltaY = (ry + tY) - cy; const ux = getStyleNum(n, '--ux'), uy = getStyleNum(n, '--uy'), rot = getStyleNum(n, '--rot'); n.style.setProperty('--ux', (ux + deltaX) + 'px'); n.style.setProperty('--uy', (uy + deltaY) + 'px'); n.style.setProperty('--rot', (rot + dAng2) + 'deg'); }); }); mergeGroups(anchor, o); snapped = true; linkSides(anchor, sideA, o, sideB);
        }
    ...
    function resetAfterSolveWithFade() { document.body.classList.remove('puzzle-solved'); const modal = document.getElementById('puzzleModal'); if (modal) modal.remove(); const cards9 = Array.from(document.querySelectorAll('.polaroid')).slice(0, 9); cards9.forEach(c => { c.style.transition = 'opacity .45s ease'; c.style.opacity = '0'; }); setTimeout(() => { groups.clear(); groupSeq = 1; cards9.forEach(c => { unlinkAll(c); c.classList.remove('dragging', 'rotating'); c.dataset.group = ''; ensureGroup(c); c.dataset.flipped = '0'; delete c.dataset.didDrag; c.style.setProperty('--ux', '0px'); c.style.setProperty('--uy', '0px'); c.style.setProperty('--rot', '0deg'); delete c.dataset.seededRot; delete c.dataset._baseRot; updateFlipper(c); updateRotor(c); }); preSeedRotations(); scatter(); requestAnimationFrame(() => { cards9.forEach(c => { c.style.opacity = '1'; setTimeout(() => { c.style.transition = ''; }, 500); }); }); }, 460); }
