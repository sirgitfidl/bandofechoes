...
(function () {
    ...
    function scatter() {
        ...
        if (!collage) return; const items = collage.querySelectorAll('.polaroid'); const vw = window.innerWidth || document.documentElement.clientWidth; const mobileMode = vw <= 560; let repW = 0, repH = 0; if (mobileMode) { const first = items[0]; if (first) { repW = first.offsetWidth || 200; repH = repW * 1.22; const targetH = Math.round(repH * 1.95); collage.style.position = 'relative'; collage.style.height = targetH + 'px'; } } else { if (collage.style.height) { collage.style.height = ''; collage.style.position = ''; } }
        items.forEach((el, i) => { const w = el.offsetWidth || repW || 300; const randomRot = biasedRotation; if (mobileMode) { if (el.style.position !== 'absolute') { el.style.position = 'absolute'; el.style.top = '0'; el.style.left = '0'; } const h = repH || (w * 1.22); const containerW = collage.clientWidth || (w * 3); const containerH = parseFloat(collage.style.height) || (h * 2.1); const cx = containerW / 2; const cy = containerH / 2; const spreadX = w * 1.38; const spreadY = h * 0.82; const tri = () => (Math.random() + Math.random() - 1); const rxLocal = tri() * spreadX; const ryLocal = tri() * spreadY; const rx = cx - w / 2 + rxLocal; const ry = cy - h / 2 + ryLocal; if (!el.dataset.seededRot) { const rot = randomRot(); el.style.setProperty('--rot', rot + 'deg'); el.dataset.seededRot = '1'; el.dataset._baseRot = rot; } el.style.setProperty('--tx', rx + 'px'); el.style.setProperty('--ty', ry + 'px'); // Avoid covering more than 50% of another polaroid; jitter if needed
                let attempts = 0;
    ...
    function addRotor(el) {
        ...
        function up(e) {
            ...
            updateRotor(el);
            // After a touch rotate gesture, keep this card's controls visible on mobile
    ...
    function setSolved(v) {
        ...
        function sanitizeWitnessHref(raw) {
            const fallback = 'https://www.youtube.com/shorts/hkYhlXNTsJQ';
            try {
                ...
                if (!allowed.includes(host)) return fallback;
                return u.toString();
            } catch (_) {
                return fallback;
            }
        ...
        const buildModal = () => {
            ...
            overlay.setAttribute('data-testid', 'puzzle-modal');
            ...
            closeBtn.setAttribute('data-testid', 'puzzle-modal-close');
            closeBtn.type = 'button';
            closeBtn.textContent = '×';
            closeBtn.style.cssText = 'position:absolute;top:8px;right:8px;background:transparent;border:0;color:#bbb;font-size:22px;line-height:1;cursor:pointer';
            closeBtn.addEventListener('click', dismiss);

            const phraseEl = document.createElement('h3');
            phraseEl.id = 'pmPhrase';
            phraseEl.textContent = solvePhrase;
            phraseEl.style.cssText = 'margin:0 0 12px;font:600 20px/1.3 system-ui';

            const actions = document.createElement('div');
            actions.style.cssText = 'display:flex;gap:18px;margin-top:10px;flex-wrap:wrap;justify-content:center;align-items:center';

            const linkCMND = document.createElement('a');
            linkCMND.id = 'pmCMND';
            linkCMND.setAttribute('data-testid', 'puzzle-witness');
            linkCMND.textContent = 'WITNESS';
            linkCMND.href = sanitizeWitnessHref(window.__WITNESS_URL || '');
            linkCMND.target = '_blank';
            linkCMND.rel = 'noopener noreferrer';
            linkCMND.style.cssText = 'text-decoration:none;padding:10px 14px;border-radius:10px;background:#e7e7e7;color:#111;font-weight:700';
            linkCMND.addEventListener('click', () => { try { stopAllSiteAudio(); } catch (_) { } overlay.remove(); window.__doSolveResetOnce?.(); });

            const linkCNTRL = document.createElement('a');
            linkCNTRL.id = 'pmCNTRL';
            linkCNTRL.setAttribute('data-testid', 'puzzle-reject');
            linkCNTRL.textContent = 'REJECT';
            linkCNTRL.href = 'theseAreNotTheTracksYoureLookingFor.html';
            linkCNTRL.style.cssText = 'text-decoration:none;padding:10px 14px;border-radius:10px;background:#1b1b1b;color:#e7e7e7;font-weight:700;border:1px solid #333';
            linkCNTRL.addEventListener('click', ev => { ev.preventDefault(); try { stopAllSiteAudio(); } catch (_) { } overlay.remove(); window.__doSolveResetOnce?.(); openMixerModal(); });

            actions.appendChild(linkCMND);
            actions.appendChild(linkCNTRL);
