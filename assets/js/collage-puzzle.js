...
(function () {
    ...
    function setSolved(v) {
        ...
        const phraseList = window.PUZZLE_SOLVE_PHRASES; const solvePhrase = phraseList[Math.floor(Math.random() * phraseList.length)] || 'You found something'; const buildModal = () => { const old = document.getElementById('puzzleModal'); if (old) old.remove(); const overlay = document.createElement('div'); overlay.id = 'puzzleModal'; overlay.setAttribute('role', 'dialog'); overlay.setAttribute('aria-modal', 'true'); overlay.setAttribute('data-testid', 'puzzle-modal'); overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.72);display:flex;align-items:center;justify-content:center;z-index:2147483647;opacity:0;transition:opacity 1.5s ease;'; const box = document.createElement('div'); box.style.cssText = 'position:relative;background:#0f0f0f;border:1px solid #333;color:#fff;padding:22px;border-radius:14px;box-shadow:0 20px 80px rgba(0,0,0,.7);width:min(480px,calc(100vw - 32px));text-align:center;'; const witnessHref = (window.__WITNESS_URL || 'https://www.youtube.com/shorts/hkYhlXNTsJQ'); box.innerHTML = `<button id="pmClose" aria-label="Close" data-testid="puzzle-modal-close" style="position:absolute;top:8px;right:8px;background:transparent;border:0;color:#bbb;font-size:22px;line-height:1;cursor:pointer">×</button><h3 id="pmPhrase" style="margin:0 0 12px;font:600 20px/1.3 system-ui"></h3><div style="display:flex;gap:18px;margin-top:10px;flex-wrap:wrap;justify-content:center;align-items:center"><a id=\"pmCMND\" data-testid=\"puzzle-witness\" href=\"${witnessHref}\" target=\"_blank\" rel=\"noopener\" style=\"text-decoration:none;padding:10px 14px;border-radius:10px;background:#e7e7e7;color:#111;font-weight:700\">WITNESS</a><a id="pmCNTRL" data-testid="puzzle-reject" href="theseAreNotTheTracksYoureLookingFor.html" style="text-decoration:none;padding:10px 14px;border-radius:10px;background:#1b1b1b;color:#e7e7e7;font-weight:700;border:1px solid #333">REJECT</a></div>`; const phraseEl = box.querySelector('#pmPhrase'); if (phraseEl) phraseEl.textContent = solvePhrase; overlay.appendChild(box); document.body.appendChild(overlay); const dismiss = () => { overlay.remove(); window.__doSolveResetOnce?.(); }; box.querySelector('#pmClose').addEventListener('click', dismiss); overlay.addEventListener('click', e => { if (e.target === overlay) dismiss(); }); const onKey = e => { if (e.key === 'Escape') { e.preventDefault(); dismiss(); document.removeEventListener('keydown', onKey); } }; document.addEventListener('keydown', onKey); const linkCMND = box.querySelector('#pmCMND'); const linkCNTRL = box.querySelector('#pmCNTRL'); if (linkCMND) { linkCMND.setAttribute('target', '_blank'); linkCMND.setAttribute('rel', 'noopener'); linkCMND.addEventListener('click', () => { try { stopAllSiteAudio(); } catch (_) { } overlay.remove(); window.__doSolveResetOnce?.(); }); } if (linkCNTRL) { linkCNTRL.removeAttribute('target'); linkCNTRL.removeAttribute('rel'); linkCNTRL.addEventListener('click', ev => { ev.preventDefault(); try { stopAllSiteAudio(); } catch (_) { } overlay.remove(); window.__doSolveResetOnce?.(); openMixerModal(); }); } overlay.style.opacity = '0'; void overlay.offsetWidth; requestAnimationFrame(() => { requestAnimationFrame(() => { overlay.style.opacity = '1'; }); }); };
