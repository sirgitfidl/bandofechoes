...
test.describe('Home page', () => {
    ...
    test('hero poster click injects autoplaying YouTube iframe', async ({ mainPage }) => {
    ...
    test('nav opens/closes and escape closes; skip link is wired to #main', async ({ mainPage }) => {
    ...
    test.describe('flaky-only', () => {
        ...
        test('lightbox opens from collage and can be closed', async ({ mainPage }) => {
            await test.step('ensure overlay hidden', async () => {
                await expect(mainPage.rotateLockOverlay).toHaveAttribute('aria-hidden', 'true');
            });
    ...
    test('footer and CTA links target correct destinations', async ({ mainPage }) => {
    ...
    test('puzzle solve chord shows overlay and allows closing', async ({ mainPage }) => {
    ...
    test('mixer modal opens and closes via inline button', async ({ mainPage }) => {
